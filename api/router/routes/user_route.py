...
async def get_current_user(user_id: int, db: AsyncSession = Depends(get_db)) -> UserResponse:
    # get the user by ID
...
async def update_current_user(user_id: int, user_update: UserUpdate, db: AsyncSession = Depends(get_db)) -> UserResponse:
